...
class DayImageOnDate(object):
    ...
    def __init__(self, location, date, precision):
        print precision
    ...
    def make_drawing(self):
        self.drawing = daydrawing.make_day(self.sunrise, self.sunset, self.precision)
        return self.drawing
...
def compare_locations(locations, date, precision=4):
    ...
    for loc in locations:
        drawing = DayImageOnDate(loc, date, precision).make_drawing()
        drawing += "    " + loc + " on " + date + "\n"
...
def compare_dates(location, dates, precision=4):
    ...
    date_drawings = []
    for date in dates:
        drawing = DayImageOnDate(location, date, precision).make_drawing()
...
# print compare_dates("quito", dates, 8)
